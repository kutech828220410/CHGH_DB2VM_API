...
namespace per_order_update
{
    class Program
    {
        ...
        {
            bool isNewInstance;
            Mutex mutex = new Mutex(true, "per_order_update", out isNewInstance);
            try
            {
                ...
                while (true)
                {
                    ...
                    try
                    {
                        ...
                        Console.WriteLine($"下載慢處方資料,共<{list_src_order.Count}>筆...,{myTimerBasic}");
                        ...
                        DateTime dateTime_st = DateTime.Now.AddDays(-4);
